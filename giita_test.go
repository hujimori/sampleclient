	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expectedErrMessage == "" {
				if err != nil {
					t.Fatalf("responce error should be nil. got=%s", err.Error())
				}

				if len(items) != len(tc.expectedItems) {
					t.Fatalf("responce items wrong. want=%+v, got=%+v", tc.expectedItems, items)
				}
				...
				if err == nil {
					t.Fatalf("responce error should not be non-nil. got=nil")
				}

				if !strings.Contains(err.Error(), tc.expectedErrMessage) {
					t.Fatalf("responce error message wrong. '%s' is expected to contain '%s'", err.Error(), tc.expectedErrMessage)
				}
			}
		})
	}
